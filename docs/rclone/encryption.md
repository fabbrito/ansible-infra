...
```bash
rclone ls r2:backups/<service>/<scope>/      # encrypted filenames
rclone ls r2crypt:<service>/<scope>/         # plaintext filenames — proves decrypt works
```
...
2. Add a second stanza (`[r2crypt_v2]`) over a different upstream path (`remote = r2:backups-v2`), and render it on
   every host.
3. Per host: `rclone copy r2crypt:<service>/<scope> r2crypt_v2:<service>/<scope>` — this streams decrypt-old →
   encrypt-new client-side. It costs 2× the data in bandwidth, per host.
4. Flip each service's destination to `r2crypt_v2:` and redeploy.
5. Validate a backup **and a restore** through the new path, per service.
6. `rclone purge r2:backups`, then remove the old stanza and vault keys.
