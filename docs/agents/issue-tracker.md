...
> [!IMPORTANT] An issue is public shared history, and this repo's no-secrets, no-host-data, no-consumer-names rules
> apply to it exactly as they apply to code and commits. Never put a hostname, an IP, a credential, or a consumer's name
> in an issue title, body, or comment.
